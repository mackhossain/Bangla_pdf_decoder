...
def _font_line_svg(font_path: Path, cids: list[int], target_gid: int) -> str:
    """Render the actual embedded-font glyph sequence as one PDF-like line."""
    ...
    try:
        ...
        for gid in cids:
            ...
            pieces.append(
                f'<path d="{escape(path, quote=True)}" fill="{fill}" '
                f'transform="translate({x:.2f},{asc:.2f}) scale(1,-1)"/>'
            )
            if gid == target_gid:
                # A red baseline marker makes the target obvious even for tiny glyphs.
                advance = float(hmtx[name][0])
                pieces.append(
                    f'<line x1="{x:.2f}" y1="{asc + 70:.2f}" '
                    f'x2="{x + max(advance, upm * .12):.2f}" y2="{asc + 70:.2f}" '
                    'stroke="#d11" stroke-width="12"/>'
                )
            x += float(hmtx[name][0]) * scale
            max_x = x

        width = max(max_x + upm * 0.25, upm)
        height = max(asc - desc + upm * 0.35, upm)
        y0 = desc - upm * 0.15
        return (
            f'<svg class="line-glyphs" viewBox="0 {y0:.2f} {width:.2f} {height:.2f}" '
...
def _write_html(
    ...
    """Write exactly one self-contained review HTML file; no companion SVG."""
    mapping = mapping or {}
...
<title>EC Glyph Review — CID/GID {gid}</title>
<style>
body{{font-family:Arial,sans-serif;margin:24px;background:#f3f4f6;color:#111}}
.card{{max-width:1200px;margin:auto;background:#fff;border:1px solid #aaa;border-radius:10px;padding:22px;box-shadow:0 2px 10px #0001}}
h1{{margin-top:0;font-size:24px}}
.badge{{display:inline-block;background:#d11;color:#fff;font-weight:700;border-radius:5px;padding:5px 9px}}
.line{{margin-top:16px;border:1px solid #888;background:#fff;padding:18px;overflow:auto}}
.line-glyphs{{width:100%;height:220px;display:block;background:white}}
.textline{{margin-top:12px;font-size:28px;line-height:1.7;word-break:break-all;border-top:1px solid #ddd;padding-top:12px}}
.target{{color:#d11;background:#ffe0e0;border:2px solid #d11;border-radius:4px;padding:1px 4px;font-weight:700}}
.unknown{{color:#b11;background:#fff0f0;border-radius:3px;padding:1px 3px}}
.note{{color:#555;margin:8px 0}}
.meta{{font-family:Consolas,monospace;background:#f7f7f7;padding:10px;border-radius:6px}}
</style></head><body>
<div class="card">
<h1>EC Manual Glyph Review</h1>
<div class="meta">Target CID/GID: <span class="badge">{gid}</span></div>
<p class="note">The line below is rendered from the exact embedded PDF TrueType glyph outlines. The target CID/GID is shown in red.</p>
<div class="line">{line_svg}</div>
<div class="textline">{line_text}</div>
<p class="note">Enter the exact Unicode character/string in the console. The HTML file is deleted automatically after a successful save.</p>
</div></body></html>'''
...
def run(
    ...
    db_path: Path,
    candidate_path: Path,
    only_gid: int | None = None,
    unresolved_cids: list[int] | None = None,
    operations: list[dict] | None = None,
) -> None:
    """Review genuinely unknown glyphs; reuse identical confirmed outlines automatically."""
    ...
    # Find the complete text-show operation containing each target CID.
    ...
    for _resource, base_font, raw in embedded_fonts(pdf, page):
        ...
        gids = targets if unresolved_cids is not None else [
            g for g in used_gids(pdf, page) if g >= 120 and g not in cmap
        ]
        if only_gid is not None:
        ...
        with tempfile.TemporaryDirectory(prefix="ec_pdf_font_") as tmp:
            ...
            # Build a temporary CID->text map for the line visual.
            current_map: dict[int, str] = {}
            for entry in db.get("fonts", {}).get(fkey, {}).get("glyphs", {}).values():
                if isinstance(entry, dict) and isinstance(entry.get("gid"), int) and isinstance(entry.get("text"), str):
            ...
            for gid in gids:
                ...
                if reused is not None:
                    ...
                    remember_mapping(
                        ...
                        gkey=gkey, text=text, source="glyph_fingerprint_reuse", confidence=1.0
                    )
                ...
                while True:
                    ...
                    if answer == "m":
                        ...
                        remember_mapping(
                            db, fkey=fkey, base_font=base_font, gid=gid,
                            gkey=gkey, text=text, source="user_confirmed", confidence=1.0
                        )
                        current_map[gid] = text
                        fingerprint_index[gkey] = db["fonts"][fkey]["glyphs"][str(gid)]
                        save_database(db_path, db)
                ...
                # Delete the HTML only after a successful manual save.
                if saved:
                    try:
