...
.target{{max-width:900px;background:#fff;border:1px solid #aaa;border-radius:8px;padding:18px}}
.glyph{{width:100%;height:650px;border:1px solid #888;background:#fff}}
.note{{padding:10px 0;color:#444}}
</style></head><body>
<h1>EC Glyph Review — CID/GID {gid}</h1>
<div class="target"><h2>Actual PDF glyph</h2>
<div class="note">This is the exact outline from the embedded Bangla TrueType font. No candidate suggestions are shown.</div>
...
def run(pdf_path: Path, page: int, db_path: Path, candidate_path: Path,
    ...
    """Simple human review: show only the actual glyph, then let the user enter Unicode."""
    ...
    for _resource, base_font, raw in embedded_fonts(pdf, page):
        ...
        with tempfile.TemporaryDirectory(prefix='ec_pdf_font_') as tmp:
            ...
            for gid in gids:
                ...
                print(f'  SVG:  {svg_path.resolve()}', flush=True)
                print(f'  HTML: {html_path.resolve()}', flush=True)
                print('Look at the glyph in the browser, then enter the exact Unicode text.', flush=True)
                ...
                while True:
                    ...
                    if answer == 'm':
                        ...
                        if not text:
                            print('Empty mapping not accepted.', flush=True); continue
