"""Persistent, font-scoped learning database for custom PDF glyphs.

A CID number is not globally meaningful.  This module therefore keys learned
mappings by an SHA-256 fingerprint of the embedded font and records a second
fingerprint for the individual glyph.  Confirmed mappings are stored with
confidence=1.0 and can be reused on later PDFs using the same embedded font.
"""
...
def glyph_key(font_path: Path, gid: int) -> str:
    """Return a stable glyph fingerprint using fontTools when available."""
    try:
    ...
    try:
        ...
        parts: list[str] = [name, str(getattr(glyph, "xMin", 0)), str(getattr(glyph, "yMin", 0)), str(getattr(glyph, "xMax", 0)), str(getattr(glyph, "yMax", 0))]
        if glyph.isComposite():
            ...
            parts.append(repr(list(coords)))
            parts.append(repr(list(end_pts)))
            parts.append(repr(list(flags)))
...
def remember_mapping(
    ...
    *,
    ...
    glyphs[str(gid)] = {
        "gid": gid,
        "text": text,
        "unicode": [f"U+{ord(ch):04X}" for ch in text],
        "glyph_fingerprint": gkey,
