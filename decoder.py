...
def decode_pdf_all(pdf_path:str|Path,review:bool=False,*,mapping_path:str|Path='custom_glyph_map.json',learned_path:str|Path='learned_glyph_map.json',corrections_path:str|Path='data/bangla_corrections.json',gid:int|None=None,learned_db=None,profile:bool=False,dump:bool=False)->str:
    ...
    pdf_path=Path(pdf_path); page_count=len(PdfReader(str(pdf_path)).pages); shared_learned=_load_learned_database(Path(learned_path)) if learned_db is None else learned_db; chunks=[]
    for page in range(1,page_count+1):
        try:
            ...
            if review: _clear_learned_cache(); shared_learned=_load_learned_database(Path(learned_path))
        except Exception as exc: text=f'[PAGE {page} FAILED: {exc}]'
        chunks.append(f'<<<page:{page}>>>\n{text}\n<<<END>>>')
    return '\n'.join(chunks)
def main()->int:
    parser=argparse.ArgumentParser(description='Decode Bangladesh EC Bangla PDF text'); parser.add_argument('pdf',type=Path,nargs='?',help='PDF input (not needed with --generate-ttf)'); parser.add_argument('--page',type=int,required=False); parser.add_argument('--all-pages',action='store_true',help='decode every PDF page sequentially'); parser.add_argument('--mapping',type=Path,default=Path('custom_glyph_map.json')); parser.add_argument('--learned',type=Path,default=Path('learned_glyph_map.json')); parser.add_argument('--corrections',type=Path,default=Path('data/bangla_corrections.json')); parser.add_argument('--review',action='store_true'); parser.add_argument('--dump',action='store_true',help='dump each unresolved red glyph as an SVG named by CID into ./svgs'); parser.add_argument('--profile',action='store_true',help='print timing for each decoder stage; does not change decoding'); parser.add_argument('--gid',type=int); parser.add_argument('--text',type=Path); parser.add_argument('--json',dest='json_path',type=Path); parser.add_argument('--generate-ttf',action='store_true',help='generate a Unicode TTF from confirmed learned glyph mappings; no PDF/page required')
