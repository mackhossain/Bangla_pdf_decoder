...
def decode_pdf_all(pdf_path:str|Path,review:bool=False,*,mapping_path:str|Path='custom_glyph_map.json',learned_path:str|Path='learned_glyph_map.json',corrections_path:str|Path='data/bangla_corrections.json',gid:int|None=None,learned_db=None,profile:bool=False,dump:bool=False)->str:
    """Decode every PDF page and return page-delimited text.

    Each page is wrapped exactly as:

        <<<page:1>>>
        contents...
        <<<END>>>

    If review=True, the existing interactive reviewer is used page-by-page.
    """
...
def main()->int:
    ...
    if args.all_pages:
        try:
            combined=decode_pdf_all(args.pdf,False,mapping_path=args.mapping,learned_path=args.learned,corrections_path=args.corrections,gid=args.gid,profile=args.profile,dump=args.dump)
        except Exception as exc:
